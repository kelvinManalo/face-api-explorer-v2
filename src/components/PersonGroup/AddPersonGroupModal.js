...
function AddPersonGroupModal({addPersonGroupHandler}) {
  ...
  const addButtonClickEvent = () => {
    const data = {groupId,name,userData,recognitionModel};    
    setGroupid('');
    setName('');
    setUserData('');
    setRecognitionModel(null);
    addPersonGroupHandler(data);
    setOpen(false);
  }
  ...
  return (
    <Modal
      ...
      open={open}
      size='mini'
      trigger={<Button floated='right' icon labelPosition='left' primary size='small'><Icon name='user' /> Add Person Group</Button>}
    >
      <Modal.Header>Add Person Group</Modal.Header>
      <Modal.Content>
        <Form>
            <Form.Field>
            ...
            <Form.Field>
                ...
                <Dropdown
                    ...
                    onChange={(e,{ value }) => setRecognitionModel(value)}
                />
            </Form.Field>            
        </Form>
      </Modal.Content>
...
export default AddPersonGroupModal